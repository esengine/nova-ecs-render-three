import { Component, System } from '@esengine/nova-ecs';
 ...
 */
class ThreeMeshComponent extends Component {
    constructor(mesh) {
        super();
        this.mesh = null;
 ...
 * Three.js材质组件 - 材质配置
 */
class ThreeMaterialComponent extends Component {
    constructor(config = {}) {
        super();
        this.needsUpdate = false;
 ...
 * Three.js几何体组件 - 几何体配置
 */
class ThreeGeometryComponent extends Component {
 ...
 * Three.js相机组件 - 相机配置
 */
class ThreeCameraComponent extends Component {
    constructor(config = {}) {
        super();
        this.camera = null;
        this.needsUpdate = false;
...
class ThreeRenderSystem extends System {
    ...
    _createGeometry(geometryComp) {
        ...
        switch (geometryType) {
            case 'box':
                return new THREE.BoxGeometry(parameters.width || 1, parameters.height || 1, parameters.depth || 1);
            case 'sphere':
                return new THREE.SphereGeometry(parameters.radius || 0.5, parameters.widthSegments || 16, parameters.heightSegments || 12);
            case 'plane':
                return new THREE.PlaneGeometry(parameters.width || 1, parameters.height || 1);
            case 'cylinder':
                return new THREE.CylinderGeometry(parameters.radiusTop || 0.5, parameters.radiusBottom || 0.5, parameters.height || 1, parameters.radialSegments || 8);
            case 'cone':
                return new THREE.ConeGeometry(parameters.radius || 0.5, parameters.height || 1, parameters.radialSegments || 8);
            case 'torus':
                return new THREE.TorusGeometry(parameters.radius || 0.4, parameters.tube || 0.2, parameters.radialSegments || 8, parameters.tubularSegments || 6);
            default:
                return new THREE.BoxGeometry(1, 1, 1);
        }
    }
    ...
    _createCamera(cameraComp) {
        if (cameraComp.cameraType === 'perspective') {
            cameraComp.camera = new THREE.PerspectiveCamera(cameraComp.fov, cameraComp.aspect, cameraComp.near, cameraComp.far);
        }
        else {
            const size = 10;
            cameraComp.camera = new THREE.OrthographicCamera(-size * cameraComp.aspect, size * cameraComp.aspect, size, -size, cameraComp.near, cameraComp.far);
        }
    }
    /**
...
MaterialFactory.textureLoader = new THREE.TextureLoader();
...
class GeometryFactory {
    ...
    static createGeometry(config) {
        ...
        switch (config.type) {
            case 'box':
                geometry = new THREE.BoxGeometry(config.width || 1, config.height || 1, config.depth || 1, config.widthSegments || 1, config.heightSegments || 1, config.depthSegments || 1);
                break;
            case 'sphere':
                geometry = new THREE.SphereGeometry(config.radius || 0.5, config.widthSegments || 16, config.heightSegments || 12, config.phiStart || 0, config.phiLength || Math.PI * 2, config.thetaStart || 0, config.thetaLength || Math.PI);
                break;
            case 'plane':
                geometry = new THREE.PlaneGeometry(config.width || 1, config.height || 1, config.widthSegments || 1, config.heightSegments || 1);
                break;
            case 'cylinder':
                geometry = new THREE.CylinderGeometry(config.radiusTop || 0.5, config.radiusBottom || 0.5, config.height || 1, config.radialSegments || 8, config.heightSegments || 1, config.openEnded || false, config.thetaStart || 0, config.thetaLength || Math.PI * 2);
                break;
            case 'cone':
                geometry = new THREE.ConeGeometry(config.radius || 0.5, config.height || 1, config.radialSegments || 8, config.heightSegments || 1, config.openEnded || false, config.thetaStart || 0, config.thetaLength || Math.PI * 2);
                break;
            case 'torus':
                geometry = new THREE.TorusGeometry(config.radius || 0.4, config.tube || 0.2, config.radialSegments || 8, config.tubularSegments || 6, config.arc || Math.PI * 2);
                break;
            case 'torusKnot':
                geometry = new THREE.TorusKnotGeometry(config.radius || 0.4, config.tube || 0.1, config.tubularSegments || 64, config.radialSegments || 8, config.p || 2, config.q || 3);
                break;
            case 'dodecahedron':
                geometry = new THREE.DodecahedronGeometry(config.radius || 0.5, config.detail || 0);
                break;
            case 'icosahedron':
                geometry = new THREE.IcosahedronGeometry(config.radius || 0.5, config.detail || 0);
                break;
            case 'octahedron':
                geometry = new THREE.OctahedronGeometry(config.radius || 0.5, config.detail || 0);
                break;
            case 'tetrahedron':
                geometry = new THREE.TetrahedronGeometry(config.radius || 0.5, config.detail || 0);
                break;
            case 'capsule':
                geometry = new THREE.CapsuleGeometry(config.radius || 0.3, config.length || 0.8, config.capSegments || 4, config.radialSegments || 8);
                break;
            case 'ring':
                geometry = new THREE.RingGeometry(config.innerRadius || 0.2, config.outerRadius || 0.5, config.thetaSegments || 8, config.phiSegments || 1, config.thetaStart || 0, config.thetaLength || Math.PI * 2);
                break;
            case 'lathe':
                ...
                geometry = new THREE.LatheGeometry(points, config.segments || 12, config.phiStart || 0, config.phiLength || Math.PI * 2);
                break;
            case 'extrude':
                if (config.shape) {
    ...
    static createLight(config) {
        ...
        switch (config.type) {
            ...
            case 'point':
                ...
                return pointLight;
            case 'spot':
                const spotLight = new THREE.SpotLight(color, config.intensity, config.distance, config.angle, config.penumbra, config.decay);
...
class CameraFactory {
    ...
    static createCamera(config) {
        switch (config.type) {
            case 'perspective':
                return new THREE.PerspectiveCamera(config.fov || 75, config.aspect || 1, config.near || 0.1, config.far || 1000);
            case 'orthographic':
                const size = config.size || 10;
                const aspect = config.aspect || 1;
                return new THREE.OrthographicCamera(-size * aspect, size * aspect, size, -size, config.near || 0.1, config.far || 1000);
            case 'stereo':
...
class CameraUtils {
    ...
    static createFromPreset(presetName) {
        ...
        if (config.rotation) {
            camera.rotation.set(THREE.MathUtils.degToRad(config.rotation.x), THREE.MathUtils.degToRad(config.rotation.y), THREE.MathUtils.degToRad(config.rotation.z));
        }
        if (config.target) {
    ...
    static animateCamera(camera, fromConfig, toConfig, duration = 1000, onComplete) {
        ...
        const endPos = new THREE.Vector3(toConfig.position?.x || 0, toConfig.position?.y || 0, toConfig.position?.z || 0);
        let startFOV = 75;
        let endFOV = 75;
        if (camera instanceof THREE.PerspectiveCamera) {
            startFOV = camera.fov;
